...
`main.py`
和
`event_bus.py`，是理解整个“大脑”服务 ** 如何“活”起来 ** 的关键。
...
加载所有配置。
3. ** 依赖注入(装配): ** ** [核心] ** 调用
`services / service_provider.py`，根据配置，创建所有核心模块和服务（`StateManager`, `DecisionMaker`, `LLMService`
等）的 ** 单例实例 **。
4. ** 模块订阅: ** 遍历所有已实例化的模块，并调用它们的
`subscribe_to_events()`
方法，将它们“挂载”到事件总线上。
5. ** API路由挂载: ** 将
`api / `目录中定义的WebSocket和HTTP路由，挂载到FastAPI
app上。
6. ** 后台任务启动: ** 创建并启动
`StateManager`
的
...
** 本项目严禁模块间的直接方法调用！ ** 例如，`DecisionMaker` ** 永远不应该 ** 写出
`llm_service.invoke()`
这样的代码。

### 为什么？ (The "Why")
- ** 解耦(Decoupling): ** 如果直接调用，`DecisionMaker`
就与
`LLMService`
的具体实现 ** 紧密耦合 ** 了。未来更换
`LLMService`，就可能需要修改
`DecisionMaker`。
- ** 异步(
    Asynchronicity): ** 直接调用通常是阻塞的。事件驱动允许我们将耗时的任务（如API请求）“扔”到总线上，然后立刻去处理其他事情，不会阻塞主流程。
- ** 可观测性(
    Observability): ** 所有的系统活动，都被显式地、结构化地表达为“事件”。这使得我们可以轻易地在总线上加入日志、监控和调试的“探针”。

### 工作流 (The Workflow)

1. ** 发布者(Publisher): **
- 一个模块（如
`DecisionMaker`）想要执行一个动作。
- 它会 ** 实例化一个 ** 在
`schemas / events.py`
中定义的、具体的事件对象（如
`InvokeLLMEvent`）。
...
2. ** 订阅者(Subscriber): **
- 另一个模块（如
`LLMService`）在其初始化时，就已经向
`EventBus`“注册”了它感兴趣的事件。
